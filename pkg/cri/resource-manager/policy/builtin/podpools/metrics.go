...
func (p *podpools) PollMetrics() policy.Metrics {
	...
	for _, pool := range p.pools {
		...
		policyMetrics.PoolMetrics[pool.PrettyName()].CPUMilliSize = strconv.Itoa(pool.CPUs.Size() * 1000)
...
func (p *podpools) CollectMetrics(m policy.Metrics) ([]prometheus.Metric, error) {
	...
	cpuMetrics := updateCPUUsageMetrics()
...
func updateCPUUsageMetrics() []prometheus.Metric {
	cpuTimeStat.RLock()
	defer cpuTimeStat.RUnlock()
	cpuUsage := make([]prometheus.Metric, cpuCount)
	for i := 0; i < cpuCount; i++ {
		cpuUsage[i] = prometheus.MustNewConstMetric(
...
func updatePoolCPUUsageMetrics(ppm *Metrics) ([]prometheus.Metric, error) {
	...
	for index, poolName := range poolNames {
		...
		if poolDeltaTotalTime != 0 {
			poolCPUUsageList[poolName] = (1.0 - float64(poolDeltaIdleTime)/float64(poolDeltaTotalTime)) * 100.0 * float64(len(ppm.PoolMetrics[poolName].CPUIds))
		}
		poolCPUUsageMetrics[index] = prometheus.MustNewConstMetric(
			...
			ppm.PoolMetrics[poolName].CPUMilliSize,
